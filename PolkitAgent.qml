...
Item {
  ...
  // A hardware authenticator appears in the polkit PAM stack.
  property bool hardwareAuthConfigured: false
  ...
  // Keep hardware authentication visible until PAM requests a text response.
  readonly property bool hardwareAuthMode: hardwareAuthConfigured && !laptopClosed && dialogVisible && !responseRequired && !submitted && !errorFlash
  readonly property int cardHeight: panel.height > 0 ? Math.min(hardwareAuthMode ? Style.space(124) : fieldHeight + contentMargin * 2, panel.height - Style.gapsOut * 2) : (hardwareAuthMode ? Style.space(124) : fieldHeight + contentMargin * 2)
  ...
  function loadPamConfig(raw) {
    hardwareAuthConfigured = PolkitModel.hardwareAuthConfiguredFromPamConfig(raw)
    fidoConfigured = String(raw || "").indexOf("pam_u2f.so") !== -1
  }
  ...
  function refocus() {
    ...
    // In hardware-auth mode there is no field to type into — park focus on the
    // key catcher so Escape still cancels; otherwise focus the password field.
    if (hardwareAuthMode) keyCatcher.forceActiveFocus()
  ...
  FileView {
    ...
    onLoadFailed: {
      root.hardwareAuthConfigured = false
  ...
  PanelWindow {
    ...
    BorderSurface {
      ...
      Column {
        ...
        visible: root.hardwareAuthMode

        OpticalGlyph {
          anchors.horizontalCenter: parent.horizontalCenter
          width: Math.round(root.fieldHeight * 0.7)
          height: width
          text: root.fidoConfigured ? "\ue23f" : "\udb80\ude37"
        ...
        Text {
          anchors.horizontalCenter: parent.horizontalCenter
          text: root.fidoConfigured ? "Touch your security key" : "Touch your fingerprint"
